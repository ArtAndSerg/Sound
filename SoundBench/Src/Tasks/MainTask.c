...
void showError(char *text, int errCode)
{
   ...
   lcdPrintf(1, 55, clWhite, clBlack, "(код %d)", errCode);
   lcdUpdate();
   osDelay(1000);
...
void dirToDisplay(void)
{
    ...
    while ((res = f_opendir(&dir, currPath)) != FR_OK) {                           
        showError("открытия папки", (int)res);
        InitFAT();
    }    
    lcdClearAll();
    while (res == FR_OK) {
        ...
        if (res != FR_OK) {
            showError("поиска файлов", (int)res);
            break;
        }
...
int executeFile(char *fileName)
{
    ...
    if(res != FR_OK) { 
        showError("открытия файла", (int)res);
        myFree((void**)&buf);
        return 1; 
    } 
...
int myMalloc(void **buf, uint32_t size, uint32_t timeout) {
    ...
    showError("выделения памяти", size);
    return 0;
}
//-----------------------------------------------------------------------------
